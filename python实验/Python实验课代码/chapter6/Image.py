...
class Image:
    ...
    if USE_GETATTR:
        def __getattr__(self, name):  # 可以用对象名字.属性访问属性
            """
            >>> image = Image(10, 10)
            >>> len(image.colors) == 1
            True
    ...
    def resize(self, width=None, height=None):
        ...
        if height is None:
            height = self.height
        if width >= self.width and height >= self.height:  # 向外扩展，扩展的颜色为背景色
            self.__width = width
            self.__height = height
            return True
        self.__width = width
    ...
    def print_image(self, border_color, square_color):
            i = 0
            while i < self.__height:
                j = 0
                while j < self.__width:
                    if self[j, i] == border_color:
                        print('=', end=' ')
                    elif self[j, i] == square_color:
                        print('+', end=' ')
                    else:
                        print(' ', end=' ')
                    j += 1
